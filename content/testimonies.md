---
title: "Témoignages"
date: 2025-11-29
draft: false
---

## Recommandations

*Également disponibles sur mon [profil LinkedIn](https://www.linkedin.com/in/madi-mnemoi-charles-meldhine-python/details/recommendations/)*

> **[Guillaume Astor](https://fr.linkedin.com/in/guillaumeastor)** — *Data Engineering Manager chez SFEIR | Google Cloud Certified*
>
> J'ai eu le plaisir de travailler avec Charles chez Servier pendant 1 an, en particulier sur le projet Knowledge Bot. 
>
> Il a joué un rôle clé dans la migration du prototype vers une API FastAPI robuste, ainsi que dans le monitoring des pipelines d'indexation qui garantissent aujourd'hui la qualité et la fraîcheur des données. 
>
> Charles est quelqu'un de rigoureux, autonome, et toujours partant pour améliorer l'existant. 
>
> Sa maîtrise de Python, du cloud GCP, de la CI/CD sont un atout pour n'importe quelle équipe. C'est un collaborateur fiable, impliqué et très agréable au quotidien. Je le recommande sans hésiter.

---

> **[Ghislain Sambandam](https://fr.linkedin.com/in/ghislain-sambandam)** — *Consultant BI / Big Data chez Servier*
>
> J'ai eu l'occasion de collaborer avec Charles sur le projet Lead Times chez Servier. 
>
> Charles a su travailler efficacement avec les métiers, clarifier les indicateurs critiques et rendre les analyses accessibles aux équipes internationales. 
>
> Je recommande vivement Charles pour toute mission en développement Python ou Data.

---

> **[Jérémy Guilbert](https://www.linkedin.com/in/jrmyguilbert)** — *Quality Assessment Engineer chez Stormshield*
>
> J’ai pu collaborer avec Charles sur le projet open source Eternaltwin depuis plusieurs années maintenant, au sein d'une association qui maintient 12 applications web. 
>
> Il a toujours été très impliqué, présent sur tous les sujets et force de proposition. 
> Il est constant, fiable et n’hésite pas à plonger dans n’importe quelle partie du code ou de l’infrastructure quand il faut débloquer un sujet, même hors de son périmètre. 
>
>Je le recommande sans hésiter pour toute mission en développement Python ou ingénierie logicielle.

---

> **[Anthony Cyrille](https://fr.linkedin.com/in/anthony-cyrille)** — *CTO & Formateur | +2000 devs seniors formés en 2 ans | 13 ans d'exp | DDD, Tests Unitaires, Clean Code*
>
> J'ai eu l'occasion de former Charles à plusieurs pratiques avancées en matière d'ingénierie logicielle telles que la Clean Architecture et le Test-Driven Development.  
>
> Je l'ai vu progresser en profondeur sur ces sujets et appliquer ces principes avec sérieux dans ses projets. Son approche est structurée, pragmatique et orientée qualité.  
>
> Il pose de bonnes questions, challenge les choix techniques avec bienveillance et pédagogie, et s'investit réellement pour monter en compétence.  
>
> Je le recommande sans hésitation pour des missions de développement Python où la rigueur et la qualité logicielle sont essentielles.

---

## Citations

> **[Guillaume Ligner](https://fr.linkedin.com/in/guillaume-ligner-data-scientist-independant)** — *Senior Machine Learning Engineer (L'Oréal / Servier)*
>
> J'ai rarement travaillé avec un "junior" aussi mûr.

---

> **[Valentin Schiaffino](https://www.linkedin.com/in/vschiaffino/)** — *Software Engineer chez Chevron, contributeur eMush*
>
> I want to leave my job now [...] The code is really ugly compared to here.

---

> **Sylvain Breton** — *Chef de projet et développeur eMush*
>
> Je te l'ai déjà répété plusieurs fois, mais ça a été un vrai plaisir de bosser avec toi.

---

> **Contributeur eMush**
>
> Gotta tell you it's been a pleasure doing all this code.
>
> [...]
>
> Most of the code I did was created based on looking for the most similar feature, learning how it works and adjusting to what I intend to make. 
>
> [...]
>
> It was satisfying to finish one more part of the code reading the tests resulting in no errors.
>
> [...]
>
> Thanks, Charles, for taking time reviewing all my code, teaching me some good practices and doing the frontend. Your supervision was one of the reasons that kept me motivating to progress.

---

> **Contributrice eMush**
>
> Tu es le meilleur développeur que j'aie pu croiser.

---

> **Contributrice eMush**
>
> J'ai beaucoup, beaucoup appris à tes côtés.
